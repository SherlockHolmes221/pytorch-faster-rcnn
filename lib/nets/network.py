...
class Network(nn.Module):
    ...
    def _crop_pool_layer(self, bottom, rois, max_pool=True, pooling_size=7):
    ...
    def _predict(self):
        ...
        # print(net_conv.size(2), net_conv.size(3))
    ...
    def _predict_1(self, using_sp_map=False, pooling_size=7, avg_pooling=True):
        # This is just _build_network in tf-faster-rcnn
        torch.backends.cudnn.benchmark = False
        net_conv = self._image_to_head()
        # print(net_conv.size()) # torch.Size([1, 1024, 38, 50])

        if using_sp_map:
            coord_x, coord_y = np.meshgrid(np.arange(net_conv.shape[-1]), np.arange(net_conv.shape[-2]))
            coords = np.stack((coord_x, coord_y), axis=0).astype(np.float32)
            coords = torch.from_numpy(coords).cuda(self._device)
            x_coords = coords.unsqueeze(0).repeat(net_conv.shape[0], 1, 1, 1)  # 1 x 2 x H x W
            ans = self._crop_pool_layer(x_coords, self._union_boxes)
            ans = ans.view(ans.size()[0], -1)
            # print(ans.size())
            return ans
        # print(net_conv.size()) # torch.Size([1, 1026, 38, 50])

        if cfg.POOLING_MODE == 'crop':
            pool5 = self._crop_pool_layer(net_conv, self._union_boxes, pooling_size=pooling_size)
        else:
            assert False
            pool5 = self._roi_pool_layer(net_conv, self._union_boxes)
        # print(pool5.size()) # torch.Size([733, 1024, 14, 14])
        fc7 = self._head_to_tail(pool5, avg_pooling=avg_pooling)
        return fc7

    def _predict_feature_map(self):
        # This is just _build_network in tf-faster-rcnn
        torch.backends.cudnn.benchmark = False
        net_conv = self._image_to_head()

        if cfg.POOLING_MODE == 'crop':
            pool5 = self._crop_pool_layer(net_conv, self._union_boxes)
        else:
            pool5 = self._roi_pool_layer(net_conv, self._union_boxes)
        fc7 = self._head_to_tail_feature_map(pool5)
        return fc7
    ...
    def forward_1(self, image, im_info, union_boxes, gt_boxes=None,
                  mode='TRAIN', using_sp_map=False, pooling_size=7, avg_pooling=True):
        # self._image_gt_summaries['image'] = image
        # self._image_gt_summaries['gt_boxes'] = gt_boxes
        # self._image_gt_summaries['im_info'] = im_info
        ...
        self._im_info = im_info  # No need to change; actually it can be an list
        self._gt_boxes = torch.from_numpy(gt_boxes).to(self._device) if gt_boxes is not None else None
        self._union_boxes = union_boxes

        self._mode = mode

        fc7 = self._predict_1(using_sp_map=using_sp_map, pooling_size=pooling_size, avg_pooling=avg_pooling)
        return fc7

    def forward_roi_map(self, image, im_info, union_boxes, gt_boxes=None,
                        mode='TRAIN', using_sp_map=False):
    ...
    def create_architecture_fc7(self):
        self._init_head_tail()

    def get_fc7(self, image, im_info, union_boxes,
                using_sp_map=False, pooling_size=7, avg_pooling=True):
        self.eval()
        torch.cuda.empty_cache()
        with torch.no_grad():
            fc7 = self.forward_1(image, im_info, union_boxes, None,
                                 mode='TEST', using_sp_map=using_sp_map,
                                 pooling_size=pooling_size, avg_pooling=avg_pooling)

        return fc7

    def get_roi_feature_map(self, image, im_info, union_boxes, using_sp_map=False):
        self.eval()
        torch.cuda.empty_cache()
        with torch.no_grad():
            roi_map = self.forward_roi_map(image, im_info, union_boxes, None, mode='TEST')
        # print(roi_map.size()) #torch.Size([1, 2048, 7, 7])
        return roi_map
    ...
    def get_summary_for_finetue(self, blobs):
        self.eval()
        self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'])
        self.train()
        summary = self._run_summary_op_val()
        return summary

    def _run_summary_op_val(self):
        """
        Run the summary operator: feed the placeholders with corresponding newtork outputs(activations)
        """
        summaries = self._event_summaries
        self._event_summaries = {}
        return summaries
