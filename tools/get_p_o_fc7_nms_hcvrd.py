    ...
    for i in range(time):
        if i == time - 1:
            union_boxes_ = Variable(torch.FloatTensor(union_boxes[i * 512:, :])).contiguous().cuda().to(
                net._device)
            fc7_U_ = net.get_fc7(blobs['data'], blobs['im_info'], union_boxes_)
...
#  CUDA_VISIBLE_DEVICES=0 python tools/get_p_o_fc7_nms_hcvrd.py
if __name__ == '__main__':
    ...
    subset = "test"
    type = "human"
    file_path = f'/home/xian/Documents/code/PPDM_la/Datasets/HCVRD/cand' \
                f'/hoi_candidates_{subset}.hdf5'

    write_file_path = f'/home/xian/Documents/code/PPDM_la/Datasets/HCVRD/50/' \
                      f'/hoi_candidates_{type}_feats_{subset}.hdf5'
    file = h5py.File(file_path, "r")
    print(len(file))

    union_file = h5py.File(write_file_path, "w")

    for image_id in tqdm(file.keys()):
        data = file[image_id]["boxes_rpn_ids_hoi_idx"][()]

        person_boxes = data[:, 0:4].tolist()
        object_boxes = data[:, 4:8].tolist()

        image_path = '/home/xian/Documents/data/VG_100K/' + image_id.split("_")[1] + ".jpg"
        ...
        union_file.create_group(image_id)
        union_file[image_id].create_dataset(
            'features',
            data=fc)
